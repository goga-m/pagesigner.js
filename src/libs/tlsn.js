...
function send_and_recv(command, data, expected_response, chosen_notary, random_uid) {
...
function prepare_pms(modulus, tryno, reliable_sites, chosen_notary, random_uid) {
  isdefined(modulus)
  ...
  .then(function(handshake_objects) {
    ...
    return send_and_recv(comm, data, 'rrsapms_rhmac_rsapms', chosen_notary, random_uid)
  })
...
function negotiate_crippled_secrets(tlsn_session, chosen_notary, random_uid) {
  ...
  return send_and_recv('cs_cr_sr_hmacms_verifymd5sha', cs_cr_sr_hmacms_verifymd5sha, 'hmacms_hmacek_hmacverify', chosen_notary, random_uid)
  .then(function(reply_data) {
  ...
  .then(function(records) {
    ...
    return send_and_recv('verify_md5sha2', [].concat(md5_digest2, sha_digest2), 'verify_hmac2', chosen_notary, random_uid)
  })
...
function start_audit(modulus, certhash, name, port, headers, ee_secret, ee_pad_secret, rsapms2, chosen_notary, random_uid) {
  ...
  .then(function(handshake_objects) {
    ...
    return negotiate_crippled_secrets(tlsn_session, chosen_notary, random_uid)
  })
  .then(function() {
  ...
  .then(function(response) {
    ...
    return send_and_recv('commit_hash', commit_hash, 'pms2', chosen_notary, random_uid)
  })
  .then(function(response) {
...
TLSClientHello.prototype.__init__ = function(args) {
  ...
  if (serialized) {
    ...
    if (client_random) {
      ...
      this.client_random = getRandom(32)
...
TLSConnectionState.prototype.aes_cbc_mpe = function(cleartext, rec_type) {
  ...
  if (this.tlsver.toString() === tls_ver_1_0.toString()) {
    ...
    this.IV = getRandom(aes_block_size) //#use a new, random IV for each record
  }
  this.seq_no += 1
  return ciphertext
}
...
TLSNClientSession.prototype.set_auditee_secret = function() {
  ...
  if (!this.auditee_secret) {
    this.auditee_secret = getRandom(this.n_auditee_entropy)
...
TLSNClientSession.prototype.get_p_value_ms = function(ctrprty) {
  ...
  for (var j = 0; j < garbage.length; j++) {
    ...
    var tmp2 = [].concat(tmp.slice(0, start), getRandom(end - start), tmp.slice(end))
    tmp = tmp2
  }
  return tmp
}
